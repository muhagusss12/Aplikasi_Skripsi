...
def start_firewall():
    try:
        ...
        subprocess.run(['iptables', '-t', 'mangle', '-A', 'PREROUTING', '-m', 'conntrack', '--ctstate', 'INVALID', '-j', 'DROP'], stderr=subprocess.STDOUT, text=True)
        subprocess.run(['iptables', '-t', 'mangle', '-A', 'PREROUTING', '-p', 'tcp', '!', '--syn', '-m', 'conntrack', '--ctstate', 'NEW', '-j', 'DROP'], stderr=subprocess.STDOUT, text=True)
        subprocess.run(['iptables', '-t', 'mangle', '-A', 'PREROUTING', '-p', 'tcp', '-m', 'conntrack', '--ctstate', 'NEW', '-m', 'tcpmss', '!', '--mss', '536:65535', '-j', 'DROP'], stderr=subprocess.STDOUT, text=True)
